...
fitted_model = f"FittedModel: {a0} + {a1}x + {a2}x^2"
print(fitted_model)

# Plot the original data and the fitted curve
plt.scatter(x, y, color="blue", label="Data Points")
plt.plot(x, a0 + a1 * x + a2 * x**2, 'r', label=f"{a0:.2f} + {a1:.2f}x + {a2:.2f}x^2", color="orange")
plt.legend()
plt.show()

# Error calculation
absolute_error = np.sum((a0 + a1 * x + a2 * x**2 - y) ** 2)
